...
public class FromTag extends SimpleTagSupport {
    ...
    private void createHtml() throws IOException {
        ...
        String submitUrl = String.valueOf(this.getJspContext().findAttribute(Cost.REQUEST_URL));
        html.append("<form id='form1' method='").append("get".toUpperCase().equals(requestType.toUpperCase()) ? "get" : "post").append("'");
        if (!StringUtils.isEmpty(submitUrl)) {
            html.append("acttion='").append(submitUrl).append("'").append(">");
            html.append("<script>").append("var fromUrl = '" + submitUrl + "';").append("var requestType =").append("get".toUpperCase().equals(requestType.toUpperCase())?"'get'":"'post'").append(";").append("</script>");
        } else
            html.append(">");
        ...
        html.append("<script>").append("  $(\".sqlTime\").datetimepicker({format: 'yyyy-MM-dd',  language:  'zh'});\n" +
                "    $(\".DateTime\").datetimepicker({format: 'yyyy-MM-dd HH:mm:ss',  language:  'zh'});").append("</script>");
    ...
    private String createNode(int index) {
        ...
        Field field = conditionFields == null ? null : (index >= conditionFields.length) ? null : conditionFields[index];
        html.append("<div class=\"col-md-6\">").append("<div class=\"form-group \">").append("<label class=\"col-sm-3 control-label text-right\" for=\"formGroupInputSmall\">" + labelName + charStr + "</label>");
        if (field == null) {
            html.append("<div class=\"col-sm-9\">");
            html.append("<input class=\"form-control\" value='" + getFieldValue(inputName) + "' name=\"" + inputName + "\" type=\"text\" id=\"" + inputName + "\" placeholder=\"" + labelName + "\"/>");
            html.append(" </div>");
        } else {
    ...
    private String createTime(Field field, String inputName, String titleName) {
        ...
        if (field.getType().equals(java.util.Date.class)) {
            html.append(" <input class=\"form-control DateTime\" type=\"text\" date-language='zn_CN' name='" + inputName + "' value='" + getFieldValue(inputName) + "' id=\"" + inputName + "\" placeholder=\"" + titleName + "\">");
            html.append(" <span class=\"add-on\"><i class=\"icon-th\"></i></span>");
        } else if (field.getType().getClass().equals(java.sql.Date.class)) {
            html.append(" <input class=\"form-control sqlTime\" date-language='zn_CN' type=\"text\" value=" + getFieldValue(inputName) + " name='" + inputName + "' id=\"" + inputName + "\" placeholder=\"" + titleName + "\">");
            html.append(" <span class=\"add-on\"><i class=\"icon-th\"></i></span>");
        } else {
            html.append(" <input class=\"form-control\" type=\"text\" value='" + getFieldValue(inputName) + "' name='" + inputName + "' id=\"" + inputName + "\" placeholder=\"" + titleName + "\">");
        }
        return html.toString();
    }
