...
import com.vuck.enums.Annotation;
import com.vuck.enums.*;
...
public class Util
{
    ...
    public static InitMsg initValue(HttpServletRequest request, Field[] Fileds, MethodParameter method)
    {
        ...
        for (Field field : Fileds)
        {
            if (!StringUtils.isEmpty(getParameterValue(request, method, field)))
            {
        ...
        throws Exception
    {
        ...
        //得到文件名的操作
        if (ExcelInfo.class.isAssignableFrom(tigerClass) &&
            !Cost.EXCEL_NO_DEFINE_FILENAME.equals(exportExcel.fileName()))
        {
        ...
        ExcelUtil
            .ExportFile(ExcelUtil.getWorkBook(), titleList, dataList, paramHttpServletResponse, fileName, dateType,
                exportExcel.cellWidth(), cell_Height);
        return true;
    }
    ...
    public static String getRequestUrl(HttpServletRequest request)
    {
        ...
        if (request == null) return url.toString();
        ...
        while (parameterNames.hasMoreElements())
        {
            ...
            int i = url.indexOf("?");
            url.append(i != -1 ? ("&" + nextElement + "=") : ("?" + nextElement + "="))
                .append(request.getParameter(nextElement));
        }
        return url.toString();
    }

    public static String getRequestUrlParam(HttpServletRequest request)
    {
        StringBuilder url = new StringBuilder();
        if (request == null) return url.toString();
        url.append(request.getRequestURI());
        ...
        while (iterator.hasNext())
        {
            ...
            for (String value : parameterMap.get(nextElement))
            {
                if (!StringUtils.isEmpty(value))
                {
                    int i = url.indexOf("?");
                    url.append(i != -1 ? ("&" + nextElement + "=") : ("?" + nextElement + "=")).append(value);
                    break;
                }
